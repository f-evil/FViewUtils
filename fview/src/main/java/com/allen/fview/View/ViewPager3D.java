...
public class ViewPager3D extends ViewPager {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		switch (action) {
			...
			case MotionEvent.ACTION_POINTER_UP: {
				final int pointerIndex = (ev.getAction() & MotionEvent.ACTION_POINTER_INDEX_MASK) >> MotionEvent
	...
	protected boolean getChildStaticTransformation(View child, Transformation t) {
		...
		if (mOverscrollEffect.isOverscrolling() && isFirstOrLast) {
			...
			t.getMatrix().reset();
			final float translateZ = (float) (mOverscrollTranslation * Math.sin(Math.PI * Math.abs(mOverscrollEffect
					.mOverscroll)));
			final float degrees = 90 / mOverscrollRotation - (float) ((RADIANS * Math.acos(mOverscrollEffect
					.mOverscroll)) / mOverscrollRotation);
			...
			final float translateZ = (mSwipeTranslation * (float) Math.sin((float) ((Math.PI) *
